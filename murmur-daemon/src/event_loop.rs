...
pub async fn run_event_loop(state: Arc<NodeState>) {
    ...
    loop {
        ...
        if tick_count % 20 == 0 {
            ...
            for peer_id in active_nodes {
                if let Some(conn) = conns.get(&peer_id) {
                    // Heartbeat ping
                    let _ = conn
                        .send_message(&murmur_core::net::NetMessage::HeartbeatPing)
                        .await;
                }
            }
        }
        ...
        if tick_count % 10 == 0 {
            let (affected_manifests, offending_nodes) = state
                .tracker
                .write()
                .await
                .check_timeouts(Duration::from_secs(5));
            ...
            if !offending_nodes.is_empty() {
                ...
                for node_id in offending_nodes {
                    warn!(
                        "BANNING peer {} due to RequestChunk timeout (Slow Loris protection)",
                        node_id.0
                    );
