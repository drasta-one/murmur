...
use murmur_proto::control::{
    EventSubscribeRequest, ClusterEvent,
    ChunkVerifiedReport, OstFragmentReport, Acknowledgement,
    StartDownloadRequest, StartDownloadResponse,
    SnapshotRequest, SnapshotResponse,
    StatusRequest, StatusResponse,
    SeedRequest, SeedResponse,
    ListManifestsRequest, ListManifestsResponse,
    StopRequest, StopResponse,
    DaemonEventSubscribeRequest, DaemonEvent,
    daemon_event::Event as DaemonEventType,
    BondedFetchRequest, BondedFetchResponse,
    ChunkSpeedReport,
    FetchRangeAssignment,
...
impl ControlPlane for ControlPlaneService {
    ...
    async fn start_download(
        ...
        self.state.download_destinations.write().await.insert(manifest_id, req.url.clone());
        ...
        if is_complete {
            if let Some(manifest) = self.state.manifests.read().await.get(&manifest_id).cloned() {
                ...
                if let Err(e) = self.state.storage.reassemble_file(&manifest, &req.url).await {
    ...
    async fn status(
        ...
        let is_coordinator = self.state.coordinator.lock().await.active_coordinator() == Some(self.state.node_id);
    ...
    async fn seed(
    ...
    ) -> Result<Response<SeedResponse>, Status> {
        ...
        match tokio::fs::File::open(path).await {
            Ok(mut file) => {
                ...
                let source = murmur_core::manifest::ManifestSource::LocalFile { path: std::path::PathBuf::from(filename.clone()) };
                match murmur_core::manifest::Manifest::from_async_reader(filename.clone(), &mut file, chunk_size, source, murmur_core::types::SimTime::ZERO).await {
                    Ok(manifest) => {
                        info!("Created manifest for file: {} chunks", manifest.chunks.len());
                        
                        let mut file = tokio::fs::File::open(path).await.unwrap();
                        let mut buffer = vec![0u8; chunk_size as usize];
                        
                        self.state.storage.preallocate(manifest.id, manifest.total_size).await.unwrap();
                        
                        for chunk_meta in &manifest.chunks {
                            let mut read_bytes = 0;
                            while read_bytes < chunk_meta.size as usize {
                                let n = file.read(&mut buffer[read_bytes..(chunk_meta.size as usize)]).await.unwrap();
                                if n == 0 { break; }
                                read_bytes += n;
                            }
                            let chunk_data = &buffer[..read_bytes];
                            self.state.storage.write_chunk(manifest.id, chunk_meta.id, chunk_data, chunk_meta.offset).await.unwrap();
                        }
                        
                        self.state.manifests.write().await.insert(manifest.id, manifest.clone());
                        self.state.manifest_holders.write().await.entry(manifest.id).or_default().insert(self.state.node_id);
                        
                        self.state.tracker.write().await.start_transfer(manifest.clone());
                        let mut chunks = Vec::new();
                        for chunk_meta in &manifest.chunks {
                            self.state.tracker.write().await.mark_chunk_received(manifest.id, chunk_meta.id);
                            chunks.push(chunk_meta.id);
                        }
                        
                        let manifest_msg = murmur_core::net::NetMessage::ManifestData { manifest: manifest.clone() };
                        let bitfield_msg = murmur_core::net::NetMessage::Bitfield { manifest_id: manifest.id, chunks };
                    ...
                    Err(e) => Ok(Response::new(SeedResponse { success: false, message: format!("Failed to generate manifest: {}", e) })),
                }
            }
            Err(e) => Ok(Response::new(SeedResponse { success: false, message: format!("File not found: {}", e) })),
        }
    }
    ...
    async fn list_manifests(
        ...
        let list = manifests.values().map(|m| (m.id.0.to_string(), m.name.clone())).collect();
            ...
            loop {
                ...
                for (manifest_id, progress) in transfers {
                    let event = DaemonEvent {
                        event: Some(DaemonEventType::Progress(murmur_proto::control::TransferProgressEvent {
                ...
                for (manifest_id, download) in bonded.iter() {
                    let event = DaemonEvent {
                        event: Some(DaemonEventType::BondedProgress(murmur_proto::control::BondedFetchProgressEvent {
                            manifest_id: manifest_id.0.to_string(),
                            percentage: download.progress_percentage() as f32,
                            is_complete: download.status == crate::bonded_download::BondedStatus::Completed,
    ...
    async fn bonded_fetch(
        ...
        let nodes: Vec<_> = active_ids
            .iter()
            .filter_map(|id| {
                overlay.get_node(*id).map(|n| (*id, n.config.clone()))
            })
            .collect();
        drop(overlay);
        ...
        match crate::bonded_download::initiate_bonded_download(
            ...
            Ok((download, per_node)) => {
                ...
                self.state.bonded_downloads.write().await
                    .insert(download.manifest.id, download.clone());

                self.state.manifests.write().await
                    .insert(download.manifest.id, download.manifest.clone());

                self.state.tracker.write().await
                    .start_transfer(download.manifest.clone());

                self.state.download_destinations.write().await
                    .insert(manifest_id, req.output_path.clone());

                self.state.storage.preallocate(manifest_id, total_size).await.unwrap();

                // Broadcast ManifestData to peers participating in the download
                let manifest_msg = murmur_core::net::NetMessage::ManifestData { manifest: download.manifest.clone() };
                ...
                for (node_id, assignments) in per_node {
                    if node_id == self.state.node_id {
                        ...
                        tokio::spawn(async move {
                            let _ = crate::bonded_download::execute_local_fetch(&url, &assignments, manifest_id, state_clone.clone(), 4).await;
                            crate::bonded_download::handle_request_more_work(state_clone.clone(), manifest_id, state_clone.node_id).await;
                        });
                    } else {
                ...
                Ok(Response::new(BondedFetchResponse {
                    ...
                    node_count,
                    message: format!(
                        "Bonded download started across {} nodes",
                        node_count
                    ),
                }))
            }
