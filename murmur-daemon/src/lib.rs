...
pub async fn run_daemon(cli: Cli) -> anyhow::Result<()> {
    ...
    let state = Arc::new(NodeState::new(node_id, cli.storage_dir, cli.malicious, cli.slow_loris, wan_bps).await?);
    ...
    // Update local node config and add to overlay
    {
        ...
        let mut node = murmur_core::node::Node::new(node_id, murmur_core::node::NodeConfig::default(), murmur_core::types::SimTime::ZERO);
    ...
    let quic_addr: std::net::SocketAddr = format!("0.0.0.0:{}", if cli.p2p_port == 0 { 0 } else { cli.p2p_port + 1000 }).parse()?;
            ...
            tokio::spawn(async move {
                if let Ok(connection) = incoming.await {
                    ...
                    if let Ok((send, recv)) = connection.accept_bi().await {
                        ...
                        info!("QUIC streams opened, experimental multiplexing not fully integrated into overlay yet");
                    }
                }
            });
        }
            ...
            tokio::spawn(async move {
                loop {
                    if let Ok(stream) = tokio::net::TcpStream::connect(&peer_addr_clone).await {
                        ...
                        let msg = murmur_core::net::NetMessage::Handshake { node_id: state_boot.node_id };
                        ...
                        while let Some(msg) = rx.recv().await {
                            if let murmur_core::net::NetMessage::Handshake { node_id } = &msg {
                                ...
                                if state_conn_rx.banned_peers.read().await.contains(node_id) {
                                    tracing::warn!("Dropping dialer connection to BANNED peer {}", node_id.0);
                                    break; // Break the rx loop, closing connection
                                }
                                
                                peer_node_id = *node_id;
                                state_conn_rx.connections.write().await.insert(*node_id, conn.clone());
                                let sim_time = murmur_core::types::SimTime::ZERO;
                                let mut node = murmur_core::node::Node::new(*node_id, murmur_core::node::NodeConfig::default(), sim_time);
                                node.activate();
                                ...
                                // Phase 4: Send Bitfields for manifests we have
                                {
                                    ...
                                    for manifest in manifests.values() {
                                        let available = state_conn_rx.storage.get_available_chunks(manifest).await;
                                        if !available.is_empty() {
                                            if let Some(c) = state_conn_rx.connections.read().await.get(&peer_node_id) {
                            ...
                            if peer_node_id.0 != 0 && !state_conn_rx.connections.read().await.contains_key(&peer_node_id) {
                                tracing::info!("Connection to {} was removed from state, breaking rx loop", peer_node_id.0);
                                break;
                            }
                        }
                        
                        if peer_node_id.0 != 0 {
                            state_conn_rx.connections.write().await.remove(&peer_node_id);
                            state_conn_rx.overlay.write().await.remove_node(peer_node_id);
                            let reassigned = state_conn_rx.tracker.write().await.handle_node_disconnect(peer_node_id);
                            ...
                            for (manifest_id, chunk_id) in reassigned {
                                ...
                                if let Some(target) = active.into_iter().next() {
                                    if let Some(c) = state_conn_rx.connections.read().await.get(&target) {
                                        let _ = c.send_message(&murmur_core::net::NetMessage::RequestChunk { manifest_id, chunk_id }).await;
                                        state_conn_rx.tracker.write().await.mark_chunk_in_flight(manifest_id, chunk_id, target);
                                    }
                                }
                            }
                        }
                    ...
                    info!("Lost or failed to connect to {}. Retrying in 1s...", peer_addr_clone);
        ...
        tokio::spawn(async move {
            ...
            match event {
                murmur_net::PeerEvent::Discovered(peer) => {
                    ...
                    // Phase 5: Anti-Malicious Peer Banning
                    if state_peer_tx.banned_peers.read().await.contains(&peer_node_id) {
                        tracing::warn!("Ignoring mDNS discovery for BANNED peer {}", peer.node_id);
                        continue;
                    }
                    
                    info!("Discovered peer {} at {}:{}", peer.node_id, peer.ip, peer.port);
                    let sim_time = murmur_core::types::SimTime::ZERO;
                    let mut node = murmur_core::node::Node::new(peer_node_id, murmur_core::node::NodeConfig::default(), sim_time);
                    node.activate();
                    ...
                    if state_peer_tx.node_id.0 > peer.node_id {
                        ...
                        match tokio::net::TcpStream::connect(&addr).await {
                            Ok(stream) => {
                                ...
                                let conn = Arc::new(murmur_net::PeerConnection::new_tcp(peer.node_id, stream));
                                state_peer_tx.connections.write().await.insert(NodeId(peer.node_id), conn.clone());
                                
                                // Send handshake
                                let msg = murmur_core::net::NetMessage::Handshake { node_id: state_peer_tx.node_id };
                                if let Err(e) = conn.send_message(&msg).await {
                                    error!("Failed to send handshake to {}: {}", peer.node_id, e);
                                }
                                ...
                                // Phase 4: Send Bitfields for manifests we have
                                {
                                    ...
                                    for manifest in manifests.values() {
                                        let available = state_peer_tx.storage.get_available_chunks(manifest).await;
                                        if !available.is_empty() {
                                            if let Some(c) = state_peer_tx.connections.read().await.get(&NodeId(peer.node_id)) {
                                                let _ = c.send_message(&murmur_core::net::NetMessage::Bitfield { manifest_id: manifest.id, chunks: available }).await;
                                            }
                                        }
                                    }
                                ...
                                tokio::spawn(async move {
                                    while let Some(msg) = rx.recv().await {
                                        p2p::handle_net_message(&state_conn_rx, p_node_id, msg).await;
                                    }
                                    ...
                                    let reassigned = state_conn_rx.tracker.write().await.handle_node_disconnect(p_node_id);
                                    for (manifest_id, chunk_id) in reassigned {
                                        let holders = {
                                            let m = state_conn_rx.manifest_holders.read().await;
                                            m.get(&manifest_id).cloned().unwrap_or_default()
                                        };
                                        let mut new_target = holders.iter().find(|&&id| id != p_node_id).copied();
                                        ...
                                        if let Some(target) = new_target {
                                            if let Some(conn) = state_conn_rx.connections.read().await.get(&target) {
                ...
                murmur_net::PeerEvent::Lost(id) => {
                    ...
                    let reassigned = state_peer_tx.tracker.write().await.handle_node_disconnect(p_node_id);
                    for (manifest_id, chunk_id) in reassigned {
                        let holders = {
                        ...
                        if let Some(target) = new_target {
                            if let Some(conn) = state_peer_tx.connections.read().await.get(&target) {
                                let _ = conn.send_message(&murmur_core::net::NetMessage::RequestChunk { manifest_id, chunk_id }).await;
                                state_peer_tx.tracker.write().await.mark_chunk_in_flight(manifest_id, chunk_id, target);
                            }
                        }
                } // end for
            } // end PeerEvent::Lost
                    ...
                    tokio::spawn(async move {
                        ...
                        if let Ok(n) = socket.read(&mut buf).await {
                            ...
                            if let Ok(Some(murmur_core::net::NetMessage::Handshake { node_id })) = 
                                murmur_net::framing::PostcardCodec::decode::<murmur_core::net::NetMessage>(&mut bytes_mut) 
                            {
                                // Phase 5: Anti-Malicious Peer Banning
                                if state_conn.banned_peers.read().await.contains(&node_id) {
                                    tracing::warn!("Rejecting incoming connection from BANNED peer {}", node_id.0);
                                    // Connection is dropped when `socket` goes out of scope.
                                    return;
                                }
                                
                                info!("Received Handshake from Node {}", node_id.0);
                                let conn = Arc::new(murmur_net::PeerConnection::new_tcp(node_id.0, socket));
                                state_conn.connections.write().await.insert(node_id, conn.clone());
                                ...
                                let msg = murmur_core::net::NetMessage::Handshake { node_id: state_conn.node_id };
                                let _ = conn.send_message(&msg).await;
                                ...
                                // Phase 4: Send Bitfields for manifests we have
                                {
                                    ...
                                    for manifest in manifests.values() {
                                        let available = state_conn.storage.get_available_chunks(manifest).await;
                                        if !available.is_empty() {
                                            if let Some(c) = state_conn.connections.read().await.get(&node_id) {
                                                let _ = c.send_message(&murmur_core::net::NetMessage::Bitfield { manifest_id: manifest.id, chunks: available }).await;
                                            }
                                        }
                                    }
                                ...
                                while let Some(msg) = rx.recv().await {
                                    ...
                                    if !state_conn.connections.read().await.contains_key(&node_id) {
                                        tracing::info!("Connection to {} was removed from state, breaking rx loop", node_id.0);
                                        break;
                                    }
                                }
