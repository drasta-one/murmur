...
pub async fn handle_net_message(state: &Arc<NodeState>, peer_id: NodeId, msg: murmur_core::net::NetMessage) {
    match msg {
        ...
        murmur_core::net::NetMessage::HeartbeatPing => {
            if let Some(conn) = state.connections.read().await.get(&peer_id) {
                let _ = conn.send_message(&murmur_core::net::NetMessage::HeartbeatAck).await;
            }
        }
        ...
        murmur_core::net::NetMessage::RequestManifest { manifest_id } => {
            ...
            if let Some(manifest) = manifests.get(&manifest_id) {
                if let Some(conn) = state.connections.read().await.get(&peer_id) {
                    ...
                    let _ = conn.send_message(&murmur_core::net::NetMessage::ManifestData { manifest: manifest.clone() }).await;
                } else {
        ...
        murmur_core::net::NetMessage::ManifestData { manifest } => {
            info!("Received manifest {} from peer {}", manifest.id.0, peer_id.0);
            ...
            if is_new {
                ...
                let msg = murmur_core::net::NetMessage::ManifestData { manifest: manifest.clone() };
            ...
            state.manifest_holders.write().await.entry(manifest.id).or_default().insert(peer_id);
            state.tracker.write().await.start_transfer(manifest.clone());
            
            try_request_pending_chunks(state, manifest.id).await;
        }
        murmur_core::net::NetMessage::RequestChunk { manifest_id, chunk_id } => {
            tracing::info!("Received RequestChunk {} for manifest {}", chunk_id.0, manifest_id.0);
            let manifests = state.manifests.read().await;
            let chunk_meta = manifests.get(&manifest_id).and_then(|m| m.get_chunk(chunk_id)).cloned();
            drop(manifests);
            
            if let Some(meta) = chunk_meta {
                if let Some(conn) = state.connections.read().await.get(&peer_id) {
                    tracing::info!("Found connection for peer {}, reading chunk...", peer_id.0);
                    match state.storage.read_chunk(manifest_id, chunk_id, meta.offset, meta.size).await {
                        Ok(Some(mut data)) => {
                            info!("Sending ChunkData {} to peer {}", chunk_id.0, peer_id.0);
                            if state.slow_loris {
                                tracing::warn!("Slow Loris mode: ignoring RequestChunk {} to stall peer {}", chunk_id.0, peer_id.0);
                                return; // keep connection open but never respond
                            }
                            if state.malicious {
                                tracing::warn!("Malicious mode: tampering with chunk data for chunk {}", chunk_id.0);
                                for byte in data.iter_mut() {
                                    *byte ^= 0xFF; // Invert all bits to corrupt the data
                                }
                            }
                            let _ = conn.send_message(&murmur_core::net::NetMessage::ChunkData { manifest_id, chunk_id, data }).await;
                        },
                        Ok(None) => {
                            info!("Chunk {} not found for peer {}", chunk_id.0, peer_id.0);
                            let _ = conn.send_message(&murmur_core::net::NetMessage::ChunkNotFound { manifest_id, chunk_id }).await;
                        },
                        Err(e) => {
                ...
                tracing::error!("RequestChunk: Chunk {} not found in manifest {}", chunk_id.0, manifest_id.0);
                if let Some(conn) = state.connections.read().await.get(&peer_id) {
                    let _ = conn.send_message(&murmur_core::net::NetMessage::ChunkNotFound { manifest_id, chunk_id }).await;
                }
            }
        }
        murmur_core::net::NetMessage::ChunkData { manifest_id, chunk_id, data } => {
            let manifests = state.manifests.read().await;
            let chunk_meta = manifests.get(&manifest_id).and_then(|m| m.get_chunk(chunk_id)).cloned();
            drop(manifests);
            ...
            if let Some(meta) = chunk_meta {
                ...
                if meta.hash == actual_hash || meta.hash == [0u8; 32] {
                    if state.storage.write_chunk(manifest_id, chunk_id, &data, meta.offset).await.is_ok() {
                        info!("Received and verified chunk {} from {}", chunk_id.0, peer_id.0);
                        state.tracker.write().await.mark_chunk_received(manifest_id, chunk_id);
                        
                        // Phase 4: Broadcast Have message to all peers
                        let msg = murmur_core::net::NetMessage::Have { manifest_id, chunk_id };
                        for (id, conn) in state.connections.read().await.iter() {
                            if *id != peer_id {
                                let _ = conn.send_message(&msg).await;
                            }
                        ...
                        let is_complete = {
                            ...
                            tracker.get_progress(manifest_id).map(|p| p.is_complete()).unwrap_or(false)
                        };
                        ...
                        if is_complete {
                            if let Some(dest) = state.download_destinations.read().await.get(&manifest_id).cloned() {
                                ...
                                let manifest = state.manifests.read().await.get(&manifest_id).cloned().unwrap();
                    ...
                    tracing::warn!("Chunk {} verification failed from {} (hash mismatch)", chunk_id.0, peer_id.0);
                    state.tracker.write().await.unmark_chunk_in_flight(manifest_id, chunk_id);
                    
                    // Phase 5: Anti-Malicious Peer Banning
                    tracing::error!("BANNING node {} for serving corrupted chunk data!", peer_id.0);
        ...
        murmur_core::net::NetMessage::ChunkNotFound { manifest_id, chunk_id } => {
            tracing::warn!("Peer {} reported chunk {} not found for manifest {}", peer_id.0, chunk_id.0, manifest_id.0);
        ...
        murmur_core::net::NetMessage::Bitfield { manifest_id, chunks } => {
            info!("Received Bitfield from {} for manifest {}", peer_id.0, manifest_id.0);
            let mut tracker = state.tracker.write().await;
        ...
        murmur_core::net::NetMessage::Have { manifest_id, chunk_id } => {
            state.tracker.write().await.register_peer_chunk(manifest_id, chunk_id, peer_id);
            try_request_pending_chunks(state, manifest_id).await;
        }
        murmur_core::net::NetMessage::AssignFetchRanges { url, manifest_id, coordinator_id, assignments } => {
            tracing::info!(
            ...
            tokio::spawn(async move {
                ...
                let msg = murmur_core::net::NetMessage::RequestMoreWork { manifest_id, node_id: state_clone.node_id };
                if let Some(conn) = state_clone.connections.read().await.get(&coordinator_id) {
                    let _ = conn.send_message(&msg).await;
                }
            });
        }
        murmur_core::net::NetMessage::RequestMoreWork { manifest_id, node_id } => {
            crate::bonded_download::handle_request_more_work(state.clone(), manifest_id, node_id).await;
        }
        msg @ murmur_core::net::NetMessage::ProxyConnect { .. } => {
            state.proxy_orchestrator.clone().handle_p2p_message(peer_id, msg).await;
        }
...
pub async fn try_request_pending_chunks(state: &Arc<NodeState>, manifest_id: murmur_core::types::ManifestId) {
    let mut tracker = state.tracker.write().await;
    ...
    for chunk_id in pending {
        if tracker.get_progress(manifest_id).map(|p| p.chunks_in_flight.len()).unwrap_or(0) >= 5 {
        ...
        if let Some(holders) = tracker.get_chunk_holders(manifest_id, chunk_id) {
            for peer in holders {
                if peer == state.node_id { continue; }
    ...
    for (chunk_id, peer) in to_request {
        if let Some(conn) = state.connections.read().await.get(&peer) {
            info!("Requesting chunk {} for manifest {} from peer {}", chunk_id.0, manifest_id.0, peer.0);
...
mod tests {
    ...
    async fn test_missing_chunk_request_returns_error() {
        ...
        let state = Arc::new(NodeState::new(NodeId(1), temp_dir.path().to_path_buf(), false, false, 50_000_000).await.unwrap());
        ...
        let peer_conn = Arc::new(murmur_net::connection::PeerConnection::new_tcp(99, client_conn));
        state.connections.write().await.insert(NodeId(99), peer_conn);
        
        let manifest_id = murmur_core::types::ManifestId::new();

        // Request chunk that doesn't exist
        handle_net_message(&state, NodeId(99), murmur_core::net::NetMessage::RequestChunk { manifest_id, chunk_id: murmur_core::types::ChunkId(100) }).await;
        
        let msg = peer_task.await.unwrap();
        match msg {
            murmur_core::net::NetMessage::ChunkNotFound { manifest_id: mid, chunk_id } => {
    ...
    async fn test_chunk_hash_verification_rejects_tampered_data() {
        ...
        let mut manifest = murmur_core::manifest::Manifest {
            ...
            file_hash: [0; 32],
            chunk_size: 1024,
            source: murmur_core::manifest::ManifestSource::LocalFile { path: std::path::PathBuf::from("test.txt") },
        ...
        handle_net_message(&state, NodeId(99), murmur_core::net::NetMessage::ChunkData { 
            manifest_id,
        ...
        assert!(!state.storage.has_chunk(manifest_id, murmur_core::types::ChunkId(0)).await);
    }
}
