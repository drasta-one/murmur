...
impl TransferTracker {
    ...
    pub fn register_peer_chunk(&mut self, manifest_id: ManifestId, chunk_id: ChunkId, node_id: NodeId) {
    ...
    pub fn remove_peer_chunk_availability(&mut self, manifest_id: ManifestId, chunk_id: ChunkId, node_id: NodeId) {
    ...
    pub fn get_rarest_pending_chunks(&self, manifest_id: ManifestId) -> Vec<ChunkId> {
        if let Some(progress) = self.active.get(&manifest_id) {
            ...
            pending.sort_by_key(|chunk_id| {
                let count = availability.and_then(|a| a.get(chunk_id)).map(|s| s.len()).unwrap_or(0);
                (count, chunk_id.0)
            });
            pending
        } else {
    ...
    pub fn get_chunk_holders(&self, manifest_id: ManifestId, chunk_id: ChunkId) -> Option<Vec<NodeId>> {
        self.global_chunk_availability
            .get(&manifest_id)
    ...
    pub fn start_transfer(&mut self, manifest: Manifest) {
        self.active.entry(manifest.id).or_insert_with(|| TransferProgress::new(manifest));
    }
    ...
    pub fn mark_chunk_received(&mut self, manifest_id: ManifestId, chunk_id: ChunkId) {
        if let Some(progress) = self.active.get_mut(&manifest_id) {
            if progress.chunks_pending.remove(&chunk_id) || progress.chunks_in_flight.remove(&chunk_id).is_some() {
                progress.chunks_received.insert(chunk_id);
            }
        }
    }

    pub fn mark_chunk_in_flight(&mut self, manifest_id: ManifestId, chunk_id: ChunkId, node_id: NodeId) {
        if let Some(progress) = self.active.get_mut(&manifest_id) {
            if progress.chunks_pending.remove(&chunk_id) {
                progress.chunks_in_flight.insert(chunk_id, (node_id, Instant::now()));
            }
        }
    }
    ...
    pub fn check_timeouts(&mut self, timeout: std::time::Duration) -> (Vec<ManifestId>, HashSet<NodeId>) {
...
mod tests {
    ...
    fn dummy_manifest() -> Manifest {
        Manifest {
            ...
            total_size: 300,
            chunks: vec![
                ChunkMeta { id: ChunkId(1), offset: 0, size: 100, hash: [0; 32] },
                ChunkMeta { id: ChunkId(2), offset: 100, size: 100, hash: [0; 32] },
                ChunkMeta { id: ChunkId(3), offset: 200, size: 100, hash: [0; 32] },
            ],
            file_hash: [0; 32],
            chunk_size: 100,
    ...
    fn test_in_flight_tracking() {
        ...
        let progress = tracker.get_progress(manifest.id).unwrap();
        assert!(!progress.chunks_pending.contains(&ChunkId(1)));
        assert_eq!(progress.chunks_in_flight.get(&ChunkId(1)).map(|(n, _)| *n), Some(NodeId(42)));
    ...
    fn test_rarest_first_scheduling() {
        ...
        let mut manifest = Manifest {
            ...
            file_hash: [0; 32],
            chunk_size: 1000,
            source: murmur_core::manifest::ManifestSource::LocalFile { path: std::path::PathBuf::from("test.bin") },
            created_at: murmur_core::types::SimTime::ZERO,
        };
        ...
        assert_eq!(pending, vec![ChunkId(2), ChunkId(3), ChunkId(1), ChunkId(0)]);
    }
}
